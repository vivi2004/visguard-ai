...
const makeClient = (api, options) => Effect.gen(function* () {
  ...
  const httpClient = (yield* HttpClient.HttpClient).pipe(options?.baseUrl === undefined ? _Function.identity : HttpClient.mapRequest(HttpClientRequest.prependUrl(options.baseUrl.toString())), options?.transformClient === undefined ? _Function.identity : options.transformClient);
  HttpApi.reflect(api, {
